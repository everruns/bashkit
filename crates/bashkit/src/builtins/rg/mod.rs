...
fn write_rg_json_match(
    ...
    //
    // serde_json serializes Object keys in lexicographic order, so the `data`
    // object ends with the alphabetically-last key (currently `path`). We
    // serialize the `data` body, strip its trailing `}`, append
    // `"submatches":[...]` (which sorts last after `path`), then close the
    // object and emit the outer envelope by hand.
    let data = json!({
        "path":{"text":filename},
        "lines":{"text":line.raw},
