...
///
/// Argument surface generated from uutils via `bashkit-coreutils-port`.
/// See `generated/tac_args.rs`.
pub struct Tac;
...
impl Builtin for Tac {
    async fn execute(&self, ctx: Context<'_>) -> Result<ExecResult> {
        ...
        {
            return execute_tac_clap(ctx).await;
        }
        #[cfg(not(feature = "clap-builtins"))]
        {
            return execute_tac_legacy(ctx).await;
        }
    }
}

#[cfg(feature = "clap-builtins")]
async fn execute_tac_clap(ctx: Context<'_>) -> Result<ExecResult> {
    ...
    // TODO(uutils-parity): implement -b/--before, -r/--regex, -s/--separator.
    // The generated parser accepts them so scripts that pass the flags are
    // not rejected outright; until the body lands we error explicitly when
    // any of them are actually requested rather than silently no-op'ing.
    if matches.get_flag("before") || matches.get_flag("regex") || matches.contains_id("separator") {
    ...
    let raw = match read_input_files(&ctx, &files).await {
        Ok(r) => r,
        Err(e) => return Ok(e),
    };
...
#[cfg(not(feature = "clap-builtins"))]
async fn execute_tac_legacy(ctx: Context<'_>) -> Result<ExecResult> {
