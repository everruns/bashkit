...
//
// Source: uutils/coreutils@4988134fa src/uucore/src/lib/features/extendedbigdecimal.rs
...
impl From<f64> for ExtendedBigDecimal {
    fn from(val: f64) -> Self {
        if val.is_nan() {
            if val.is_sign_negative() { Self::MinusNan } else { Self::Nan }
        } else if val.is_infinite() {
            if val.is_sign_negative() { Self::MinusInfinity } else { Self::Infinity }
    ...
    pub fn to_biguint(&self) -> Option<BigUint> {
        match self {
            Self::BigDecimal(big_decimal) => {
                ...
                bi.to_biguint().map(|bi| bi * BigUint::from(10u32).pow(-scale as u32))
            }
            _ => None,
        }
    }
...
impl Neg for ExtendedBigDecimal {
    ...
    fn neg(self) -> Self::Output {
        match self {
            Self::BigDecimal(bd) => {
                if bd.is_zero() { Self::MinusZero } else { Self::BigDecimal(bd.neg()) }
            }
