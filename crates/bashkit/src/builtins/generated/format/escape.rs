...
//
// Source: uutils/coreutils@782e5b907 src/uucore/src/lib/features/format/escape.rs
...
impl Base {
    ...
    fn convert_digit(self, c: u8) -> Option<u8> {
        match self {
            Self::Oct(_) => if matches!(c, b'0'..= b'7') { Some(c - b'0') } else { None }
            Self::Hex => {
                match c {
                    b'0'..=b'9' => Some(c - b'0'),
    ...
    if let Some((new_digits, rest)) = input.split_at_checked(digits as usize) {
        *input = rest;
        let ret = new_digits
            ...
            .map(|n| *n as u32)
            .reduce(|ret, n| {
                ret.wrapping_mul(Base::Hex.as_base() as u32).wrapping_add(n)
            })
            .expect("must have multiple digits in unicode string");
        char::from_u32(ret)
            .ok_or_else(|| EscapeError::InvalidCharacters(new_digits.to_vec()))
    } else {
...
pub fn parse_escape_code(
    ...
    if let [c, new_rest @ ..] = rest {
        if let b'1'..=b'7' = c {
            if let Some(parsed) = parse_code(
                rest,
                Base::Oct(OctalParsing::ThreeDigits),
            ) {
