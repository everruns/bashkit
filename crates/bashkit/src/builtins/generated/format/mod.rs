...
//
// Source: uutils/coreutils@782e5b907 src/uucore/src/lib/features/format/mod.rs
...
impl Display for FormatError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            Self::SpecError(s) => {
                write!(
                    f, "%{}: invalid conversion specification",
                    String::from_utf8_lossy(s)
                )
            }
            Self::TooManySpecs(s) => {
                write!(
                    f, "format '{}' has too many % directives",
                    String::from_utf8_lossy(s)
                )
            }
            Self::NeedAtLeastOneSpec(s) => {
                write!(f, "format '{}' has no % directive", String::from_utf8_lossy(s))
            }
            Self::EndsWithPercent(s) => {
                write!(f, "format {} ends in %", String::from_utf8_lossy(s).quote())
            }
            Self::InvalidPrecision(precision) => {
                write!(f, "invalid precision: '{precision}'")
            }
            ...
            Self::InvalidCharacter(escape_char, digits) => {
                write!(
                    f, "invalid universal character name \\{escape_char}{}",
...
impl<F: Formatter<T>, T> Format<F, T> {
    ...
    pub fn parse(format_string: impl AsRef<[u8]>) -> Result<Self, FormatError> {
        ...
        let Some(spec) = spec else {
            return Err(FormatError::NeedAtLeastOneSpec(format_string.as_ref().to_vec()));
        };
        ...
        for item in &mut iter {
            match item {
                Ok(FormatItem::Spec(_)) | Err(FormatError::EndsWithPercent(_)) => {
                    return Err(
                        FormatError::TooManySpecs(format_string.as_ref().to_vec()),
                    );
                }
