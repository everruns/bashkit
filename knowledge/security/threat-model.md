...
| TM-ISO-027 | Request-owned execution authority survives completion | A retained runtime, transport, callback, or budget handle emits late output, charges a reused request, or keeps resources after cancellation/timeout | One `ExecutionBudget` is created before initialization, propagated to every real boundary, polled during async waits/VM loops, and closed by an infallible RAII guard on every exit. Closed handles reject work; futures and byte leases release on drop. Shared adversarial evidence is defined in [Request Execution Lifecycle](request-lifecycle.md). | **MITIGATED** |
