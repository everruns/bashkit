...
| TM-DOS-109 | `sed r` output amplification | A script can read the complete stdin after every input line, synchronously growing output quadratically before interpreter-level truncation | Destination-aware sed sinks reject stdout growth at the configured output limit and lease capacity from the shared live-intermediate budget before allocation; file and in-place sinks use the live budget without inheriting the stdout cap; regression tests cover rejection and compatible below-limit output — **MITIGATED** |
