...
|---|---:|---|
| `lto = "fat"` on `[profile.release]` | **-1.68 MiB (5.3%)** | The only lever here with no functional or throughput cost — and it was still declined, on build time. Measured on this machine: a release rebuild after touching `crates/bashkit/src/lib.rs` goes 274s → 440s (+61%). `[profile.release]` keeps `thin` so day-to-day builds stay fast, the same decision recorded in `Cargo.toml`; `[profile.bench]` uses `fat` because Criterion runs rarely. Revisit if release rebuild time stops being the binding constraint, or move it into `cli-binaries.yml` alone so only CI pays. |
