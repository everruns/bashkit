...
- **Coreutils module-vendor mode** — Extends the codegen pipeline beyond `uu_app()` argument surfaces to vendor whole upstream uutils modules with a manifest and drift-detection CI. `tee`, `mktemp`, `realpath`, `stat`, and `od` are now ported through codegen, and `printf` now runs on a vendored copy of uucore's format implementation ([#1593](https://github.com/everruns/bashkit/pull/1593), [#1594](https://github.com/everruns/bashkit/pull/1594)).
- **`env` builtin via uutils virtual-env shim** — Ports the uutils env-default surface through a virtual-env shim (TM-INF-024), bringing the full upstream flag set without granting filesystem or process access ([#1592](https://github.com/everruns/bashkit/pull/1592)).
