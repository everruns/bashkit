...
- **CI secret-handling hardened (TM-INF-026)** — fork-PR examples job no longer exposes Doppler/Anthropic tokens; secrets now step-scoped with `--only-secrets <KEY>` injection and fork-guard. Homebrew tap push uses a dedicated PAT scoped to the tap repo only ([#1728](https://github.com/everruns/bashkit/pull/1728), [#1735](https://github.com/everruns/bashkit/pull/1735)).
